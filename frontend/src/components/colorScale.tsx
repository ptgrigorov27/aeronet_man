...
const ColorLegend: React.FC<ColorLegendProps> = ({ type, activateTrace }) => {
  ...
  if (type.includes("std") || type.includes("aod")) {
    markerDomain = Array.from({ length: 6 }, (_, i) =>
  ...
  const gradientColors = markerDomain
    .map((value) => markerScale(value))
    .join(", ");
  ...
  return (
    ...
    >
      <Typography
        variant="body1"
        sx={{ marginBottom: "-12px", fontSize: "0.9em" }}
      >
        Measurement:{" "}
        <strong>
          {type.toUpperCase().replace(/_/g, " ").replace("NM", "nm")}
        </strong>
      </Typography>
      <hr />
      <OverlayTrigger
          ...
          {markerTicks.map((tick, index) => (
            <Grid
              ...
              sx={{
                position: "relative",
                textAlign: "center",
                width: `calc(${parseFloat(tickPositions[1]) / 2}%)`,
              }}
            >
              <Typography
                variant="caption"
      ...
      {traceActive && (
        <>
          ...
          <Grid
            container
            justifyContent="space-between"
            sx={{ width: "300px", marginTop: "20px", margin: "0 auto" }}
          >
            <Typography
              sx={{
                fontWeight: "bold",
